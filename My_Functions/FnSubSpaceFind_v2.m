function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,dist2]=FnSubSpaceFind_v2(X,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,MaxIterEst,p)
%% RANSAC Functions:
...
for i=1:c
   disp(['Subspace#' num2str(i) '/' num2str(c)]) ;
    ...
    [PNV, Inliers,dist2{i}] = ransac_MSFreq(X(:,SelectedIndices), FitFunc, DistFunc, DegFunc, k, Thr,MaxIterEst,p);
